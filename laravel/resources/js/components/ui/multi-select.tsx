...
function isOptionsExist(groupOption: GroupOption, targetOption: Option[]) {
    for (const [, value] of Object.entries(groupOption)) {
        if (value.some(option => targetOption.find(p => p.value === option.value))) return true
    }
    return false
}
...
const MultipleSelector = ({
        ...
        const Item = (
            <CommandItem
                ...
                onSelect={(value: string) => {
                    if (selected.length >= maxSelected) { onMaxSelected?.(selected.length); return }
                    setInputValue('')
    ...
    return (
        <Command
            ref={dropdownRef}
            {...commandProps}
            onKeyDown={e => { handleKeyDown(e); commandProps?.onKeyDown?.(e) }}
            ...
            >
                <div className='flex flex-wrap gap-1'>
                    ...
                    <CommandPrimitive.Input
                        ...
                        value={inputValue}
                        disabled={disabled}
                        onValueChange={value => { setInputValue(value); inputProps?.onValueChange?.(value) }}
                        onBlur={event => { if (!onScrollbar) setOpen(false); inputProps?.onBlur?.(event) }}
                        onFocus={event => {
                            setOpen(true)
                        ...
                        className={cn(
                            ...
                            { 'w-full': hidePlaceholderWhenSelected, 'px-3 py-2': selected.length === 0, 'ml-1': selected.length !== 0 },
                            inputProps?.className
                        )}
                    />
                    <button
                        type='button'
                        onClick={() => { setSelected(selected.filter(s => s.fixed)); onChange?.(selected.filter(s => s.fixed)) }}
                        className={cn(
